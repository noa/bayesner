 ...
 * Copyright (c) 2015-2016, Nicholas Andrews
 *
 * All rights reserved.
 *
...
#ifndef __NN_TRIE_HPP__
#define __NN_TRIE_HPP__

#include <vector>
#include <set>
#include <algorithm>

#include <nn/trie_interface.hpp>
...
public:
    ...
    protected:
        ...
        typename Items::const_iterator mCurrentPos;
    ...
    protected:
        ...
        void next() {
            while (!isEnd()) {
                auto iterEnd = mCurrentNode->mItems.end();
...
class ptr_trie : public trie_interface<K,V> {
    typedef std::vector<K> seq_t;
    typedef std::pair<seq_t,V> result_t;
    typedef std::vector<result_t> results_t;
    typedef Trie<K, V> trie_t;

    trie_t dict;
    size_t EOS;

    size_t get_len(const K* seq) const {
        size_t ret = 0;
        while(seq[ret] != EOS) ++ret;
        return ret+1;
    }

    seq_t from_ptr(const K* seq) const {
        auto l = get_len(seq);
        seq_t ret;
...
public:
    ptr_trie(K _EOS) : dict(_EOS), EOS(_EOS) {}
    ~ptr_trie() {}

    const V& get_val(const seq_t& key) const override {
        return *dict.find(key.data())->second;
    }

    V& get_or_insert_val(const seq_t& key) override {
        return dict[key.data()];
    }

    bool insert(const seq_t& key, V val) override {
        return dict.insert(key.data(), val).second;
    }

    bool has_key(const seq_t& key) const override {
        return dict.hasKey(key.data());
    }

    results_t starts_with(const seq_t& key) const override {
        auto iter = dict.startsWith( key.data() );
        results_t results;
        for (; iter != dict.end(); ++iter) {
            result_t result(from_ptr(iter->first), *iter->second);
