...
class ConllCorpusReader(CorpusReader):
    ...
    def __init__(self, root, fileids, columntypes,
                 ...
                 tree_class=Tree, tagset=None, ne_types=None):
#        print("Using custom conll reader!")
    ...
    def ne_words(self, fileids=None, ne_types=None, tagset=None):
        self._require(self.WORDS, self.NE)
        if ne_types is None: ne_types = self._ne_types
        def get_ne_words(grid): # capture ne_types as local var
            return self._get_ne_words(grid, ne_types, tagset)
        return LazyMap(get_ne_words, self._grids(fileids))
    ...
    def _get_ne_words(self, grid, ne_types, tagset=None):
        ...
        for (word, ne_tag) in zip(words, ne_tags):
            if ne_tag == 'O':
                ...
                (state, ne_type) = ne_tag.split('-')
            # If it's a chunk we don't care about, treat it as O.
            if ne_types is not None and ne_type not in ne_types:
                state = 'O'
            # Treat a mismatching I like a B.
            if state == 'I' and ne_type != stack[-1].label():
                state = 'B'
            ...
            if state == 'B':
                new_ne = Tree(ne_type, [])
                stack[-1].append(new_ne)
                stack.append(new_ne)
            # Add the word token.
            stack[-1].append((word, ne_tag))
...
class ConllNECorpusReader(ConllCorpusReader):
    def __init__(self, root, fileids, ne_types=None, encoding='utf8', tagset=None):
        ConllCorpusReader.__init__(self, root, fileids, ('words','ne'), ne_types=ne_types, encoding=encoding, tagset=tagset)
